...
export const onStateChange = (state: NavigationState | undefined, setIsPlayerNeedMargin: (arg: boolean) => void) => {
	currentRouteName = navigationRef.current.getCurrentRoute().name
	if (currentRouteName === 'Routes' || currentRouteName === 'Objects' || currentRouteName === 'AboutUs' || currentRouteName === 'Settings') {
		setIsPlayerNeedMargin(true)
	} else {
		setIsPlayerNeedMargin(false)
	}
};
